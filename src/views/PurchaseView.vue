<template>
    ...
    <div v-else class="mx-auto">
        ...
        <div class=" pb-5 sm:w-11/12 mx-auto">
            ...
            <v-expansion-panels @click="planClick" color="blue bg-blue shadow w-1/2" v-model="panel" multiple
                v-for="(plan, i) in newPlans" :key="i">

                <v-expansion-panel :title=plan.name :value=plan.value>
                    <v-expansion-panel-text style="padding: 0px; ">

                        <v-container fluid>
                            <v-row justify="space-around">
                                <!-- LARGE SCREENS -->
                                <v-col class="lg:block hidden" v-for="(package_plan, i) in plan.packages" :key="i"
                                    cols="12" md="3">

                                    <div class="shadow">
                                        <v-card style="" class="mx-auto h-[150px]" color="surface-variant"
                                            :image=package_plan.imagePath max-width="300" width="270" height="170">
                                            <p
                                                class="sm:text-[50px] text-[50px]  h-full  px-4 bg-gradient-to-r from-orange-500 ">
                                        ...
                                        <v-card class="mx-auto" max-width="300" width="270">
                                            <v-list density="compact">
                                                <v-list-item v-for="(description, i) in package_plan.description"
                                                    :key="i" color="primary" class="text-start"
                                                    ...
                                                    <v-list-item-title class="text-xl  text-start text-wrap">{{
                                    ...
                                    <div class="flex" v-for="(package_plan, i) in plan.packages" :key="i">
                                        <div class=" sm:h-[460px] w-[300px]">
                                            <v-card class="mx-auto h-[150px]" color="surface-variant"
                                                image="/Images/student-pro.jpg" max-width="250" height="">
                                                <v-card-title class="bg-gradient-to-r from-orange-500 sm:py-3">{{
                                                    package_plan.feature }}</v-card-title>
                                                <p
                                                    class="sm:text-[70px] text-[50px]   px-4 bg-gradient-to-r from-orange-500 ">
                                                    {{ package_plan.bandwidth
                                                    }}<span class="text-[30px] sm:text-[40px]">Mbps <span
                                                            class=" sm:block hidden text-base bg-gradient-to-r from-orange-500">Ksh.
                                                            {{ package_plan.fee
                                                            }}
                                                            <span
                                            ...
                                            <v-card class="mx-auto" max-width="250" style="">
                                                <v-list density="compact">
                                                    <v-list-item v-for="(description, i) in package_plan.description"
                                                        :key="i" color="primary" style="background-color: beige;">
                                                        <template v-slot:prepend>
...
onMounted(() => {
    if (route.fullPath == '/purchase') {
    } else if (route.fullPath == '/purchase?p=priority-data' || route.fullPath == '/purchase?p=elite-home' || route.fullPath == '/purchase?p=metred-home' || route.fullPath == '/purchase?p=standard-home') {
        panel.value = []
...
function routePlan() {
    panel.value = []
    if (route.query.p == 'priority-data') {
        panel.value[0] = 'Priority Data'
    } else if (route.query.p == 'elite-home') {
        panel.value[0] = 'Elite Home'
    } else if (route.query.p == 'metred-home') {
        panel.value[0] = 'Metred Home'
    } else if (route.query.p == 'standard-home') {
        panel.value[0] = 'Standard Home'
    }
    router.replace('/purchase')
}
